...
const Gallery = () => {
  ...
  const handleOrderNow = (designTitle) => {
    toast({
      title: "تم إضافة التصميم إلى السلة",
      description: `تم إضافة "${designTitle}" إلى سلة التسوق`,
    });
  };

  const handleEditDesign = (designTitle) => {
    navigate('/design', { state: { editingDesign: designTitle } });
  };
            ...
            <Card key={design.id} className="group overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 bg-white border-0">
              ...
              <CardContent className="p-6">
                ...
                <div className="flex space-x-2 space-x-reverse">
                  <Button 
                    onClick={() => handleOrderNow(design.title)}
                    className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white"
                  >
                    اطلب الآن
                  </Button>
                  <Button 
                    onClick={() => handleEditDesign(design.title)}
                    variant="outline" 
